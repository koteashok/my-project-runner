...
def parse_args():
    p = argparse.ArgumentParser(
        description="Proposed hybrid on Amazon (real data only)")
    p.add_argument("--data", default=os.path.join("data", "amazon",
                                                  "amazon_c4.parquet"),
                   help="path to a real Amazon review file (.parquet/.csv/.json(l)(.gz))")
...
def main():
    ...
    from dataset_amazon import load_amazon
    read_cap = None if config.MAX_SAMPLES is None else max(config.MAX_SAMPLES * 5,
                                                           config.MAX_SAMPLES)
    df = load_amazon(args.data, max_rows=read_cap)   # errors if file missing
    print(f"[amazon] loaded {len(df):,} real reviews from {args.data}")
